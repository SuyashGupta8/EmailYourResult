  ...
  function formSubmitReply(E){
       ...
       var lastRow=usingObject(sheet).lastRow; //getting last row(intiger or number) by my object method
       var formRange=sheet.getRange(lastRow,1,1,usingObject(sheet).numColumn).getValues();
          
     
         var indexFunction=usingClosure(sheet,null,"Choose your semester"); //using my closure function to get index values in spreadsheet  of (column name string, here"Choose your semester") passing argument
         ...
         var columnNameIndex=indexObj.columnNameIndex;//we will get column index for string "Choose your semester" from formspreadsheet
          ...
          indexFunction=usingClosure(sheet,null,"Enter your 5 digit roll number");
...
function gettingCellValue(sheet,rollno,columnName){  
 ...
 var indexFunction=usingClosure(sheet,rollno,columnName);
  
 var indexObj=indexFunction();
  
  var rollRowIndex=indexObj.rollRowIndex;
  var columnNameIndex=indexObj.columnNameIndex;
  var values=usingObject(sheet).values;  
...
function gettingHeaderValues(sem){  
 ...
 return (usingObject(inSheet1).headerRange); //calling usingObject function which return object
}




/*function testingObject(){                //**for testing usingObject function un comment this
  ...
  var testing=usingObject(inSheet1).numColumn;
  Logger.log(testing);
 
}*/


function usingObject(sheet)  //function is used to Access (total number of rows columns header values) by returning object
{
...
// for testing usingClosure function uncomment this
     
/*function testingClosure(){ 
   sheet1= SpreadsheetApp.openByUrl('https://docs.google.com/spreadsheets/d/1o7qXTEI9ixK_Z-Afkb8WBGlj0r1yXEaham0rxdbzRow/edit#gid=736438352'); 
                        inSheet1=sheet1.getSheetByName("secondSemester");
var funcn=usingClosure(inSheet1,80012,"Enrollment Number");
var obj=funcn();
...
function usingClosure(sheet,rollNum,columnName){
  ...
  var headerArray=  usingObject(sheet).headerRange;
  ...
  var rollvalue= sheet.getRange(1,((rollColumnIndex1)+1),usingObject(sheet).lastRow).getValues();
    
  if(rollNum !== null)  
  {
    for(var i=0;i<usingObject(sheet).lastRow;i++){  //getting location of roll number (mainly row value)
